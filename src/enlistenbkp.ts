      ...
      [
        ...
        function stepListDtxTokenRegistry(authToken, step) {
          listDtxTokenRegistry(authToken).then(response => {
            console.log(
              'Step DtxTokenRegistry succesfull, response was: ',
              response
            );
        ...
        function stepListStreamRegistry(authToken, tokenAddress, step) {
          listStreamRegistry(authToken).then(response => {
            console.log(
              'Step ListStreamRegistry succesfull, response was: ',
              response
            );
        ...
        function stepApproveDtxAmount(
          ...
          console.log(
            `Approving spender ${spenderAddress}, on token ${tokenAddress} for amount ${
              sensor.stakeamount
            }`
          );
          approve(authToken, tokenAddress, spenderAddress, sensor.stakeamount)
            .then(response => {
              console.log(
                'Step stepApproveDtxAmount succesfull, response was: ',
                response
              );
        ...
        function stepAwaitApproval(authToken, tokenAddress, uuid, step) {
          const url =
            rtrim(baseUrl, '/') + `/dtxtoken/${tokenAddress}/approve/${uuid}`;
          waitFor(authToken, url).then(response => {
            console.log(
              'Step AwaitApproval succesfull, response was: ',
              response
            );
            step(null, authToken);
          });
        ...
        function stepEnlistSensor(authToken, step) {
          enlist(authToken, sensor).then(response => {
            console.log(
              'Step EnlistSensor succesfull, response was: ',
              response
            );
        ...
        function stepAwaitEnlisting(authToken, uuid, step) {
          const url = rtrim(baseUrl, '/') + `/sensorregistry/enlist/${uuid}`;
          waitFor(authToken, url).then(response => {
            console.log(
              'Step AwaitEnlisting succesfull, response was: ',
              response
            );
            step(null);
          });
...
async function enlist(authToken, sensor) {
  return rp({
    method: 'POST',
    uri: rtrim(baseUrl, '/') + '/sensorregistry/enlist',
    body: {
